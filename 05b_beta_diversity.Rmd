...
### Rarefied clr distance matrix {#center-log-ratio-matrix}
```{r}
gm <- function(x){
  exp(mean(log(x[x>0])))}
...
inner_join(norare_dtbl, rare_dtbl, by=c("sample_id", "name")) %>%
  ...
  dplyr::select(sample_id, name, norare=value.x, rare=value.y, rclr=value.x.x, zclr=value.y.y, diffs) %>%
