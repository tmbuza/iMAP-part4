...
nseqs_per_sample <- readRDS("../iMAP-part3/RDataRDS/phylo_table_long.rds") %>% 
  group_by(sample_id) %>%
...
### Computing group p-values and p-adjusted
```{r, fig.dim=c(7, 6)}
...
if(min(group_signif$p.value) <= 0.5) {
  group_signif <- group_signif %>% 
    filter(p.value <= 0.5)
  
paste("Found significant P-values ranging from:", min(group_signif$p.value), "-", max(group_signif$p.value))
...
### Defining x-y coordinates and stars
pairs_test_x_y <- pairs_test %>% 
  ...
  mutate(pos = as.numeric(taxon),
         ...
         x = case_when(
           ...
           # group1 == "Wildebeest" &
           #   group2 == "Buffalo" ~ max_quartile * 1.5),
           # group1 == "Wildebeest" &
           #   group2 == "Warthog" ~ max_quartile * 1.9,
           # group1 == "Zebra" &
           #   group2 == "Buffalo" ~ max_quartile * 2.2,
           # group1 == "Zebra" &
           #   group2 == "Warthog" ~ max_quartile * 2.5,
           # group1 == "Zebra" &
           #   group2 == "Wildebeest" ~ max_quartile * 2.8),
         xend = x,
         x_star = 1.15*x,
         y_star = case_when(
           group1 == "Warthog" &
...
pairs_test %>%
  ...
  paste("P-values range from:", min(group_signif$p.value), "-", max(group_signif$p.value))
}
