...
metadata <- readRDS("../smda-end2end/RDataRDS/mo_metadata.rds") %>% 
  select(sample_id, group = isolate) %>% 
  drop_na(group)

phylo_sub_counts_long <- read_tsv("../smda-end2end/data/final.tx.1.subsample.shared", show_col_types = F) %>%
  rename_all(tolower) %>% 
...
taxonomy <- read_tsv("../smda-end2end/data/final.tx.1.cons.taxonomy", show_col_types = F) %>%
  rename_all(tolower) %>%
  select(otu, taxonomy) %>%
...
otuplot <- otu_group_rel_abund %>% 
  mutate(rel_abund = if_else(rel_abund == 0, (3/4)*lod, rel_abund)) %>%
  ...
  scale_color_manual(name = NULL,
                    breaks = c("Buffalo", "Wildebeest", "Warthog", "Zebra"),
                    labels = c("Buffalo", "Wildebeest", "Warthog", "Zebra"),
                    values = c("blue2", "red", "green4", "purple")) +
labs(y = NULL,
...
```{r fig.dim=c(7, 6), message=FALSE, warning=FALSE}
...
if(min(expt_signif$p.value) < 0.2) {
  expt_signif <- expt_signif %>% 
    ...
    saveRDS(expt_signif, "RDataRDS/signif_pairs.rds")
...
cat("\nSignificant pairs\n")
readRDS("RDataRDS/pairs_test.rds") %>% 
  select(-max_quartile, 
         Taxon = taxon, 
         Group1 = group1, 
         Group2 = group2,
